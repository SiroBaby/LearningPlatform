...
cases = {
  ...
  'wrong Grafana memory request' => ['requests: {cpu: 75m, memory: 256Mi}', 'requests: {cpu: 75m, memory: 128Mi}'],
  ...
  'missing application pod dashboard ConfigMap' => ['learning-platform-monitoring-grafana-dashboards-application-pod', 'missing-application-pod-dashboard'],
  'wrong application pod dashboard title' => ['"title":"Application / Pod"', '"title":"Wrong dashboard"'],
  'application pod dashboard cluster selector' => ['"expr":"kube_pod_status_ready{namespace=~\\"$namespace\\"', '"expr":"kube_pod_status_ready{cluster=~\\"$cluster\\",namespace=~\\"$namespace\\"'],
  'application pod dashboard container condition' => ['container=~\\"$container\\"}', 'container=~\\"$container\\",condition=\\"true\\"}'],
  'application pod dashboard phase selector' => ['phase=~\\"Running|Pending|Failed\\"', 'phase=\\"Running\\"'],
  'application pod dashboard phase aggregation' => ['sum by (phase) (kube_pod_status_phase', 'sum(kube_pod_status_phase'],
  'application pod dashboard Loki pod selector' => ['"title":"Log drilldown","targets":[{"expr":"{namespace=~\\"$namespace\\",pod=~\\"$pod\\",container=~\\"$container\\"}', '"title":"Log drilldown","targets":[{"expr":"{namespace=~\\"$namespace\\",container=~\\"$container\\"}'],
  'missing application PrometheusRule alert' => ['alert: LearningPlatformWorkerUnhealthy', 'alert: WrongWorkerAlert'],
  'application PrometheusRule pod-ready condition' => ['pod=~"api-.*|web-.*|worker-.*",condition="true"', 'pod=~"api-.*|web-.*|worker-.*",condition="false"'],
  'application PrometheusRule worker containers' => ['container=~"worker|go-worker"', 'container="worker"'],
  'application PrometheusRule worker pod selector' => ['pod=~"worker-.*"', 'pod=~"api-.*"'],
  'application PrometheusRule worker condition' => ['container=~"worker|go-worker"} == 0', 'container=~"worker|go-worker",condition="true"} == 0'],
  'application PrometheusRule pod label join' => ['condition="true"} == 0', 'condition="true"} == 0 and kube_pod_labels{namespace="learning-platform-dev"}'],
  'application PrometheusRule crashloop reason' => ['reason="CrashLoopBackOff"', 'reason="ImagePullBackOff"'],
  'application PrometheusRule deployment scope' => ['deployment=~"web|api|worker"', 'deployment="web"'],
  'application PrometheusRule delayed firing' => ["          for: 10m\n", "          for: 1m\n"],
  ...
  'resources missing' => ['requests: {cpu: 35m, memory: 64Mi}', 'requests: {}'],
  'wrong Prometheus CPU request' => ['requests: {cpu: 250m, memory: 512Mi}', 'requests: {cpu: 251m, memory: 512Mi}'],
  'wrong Prometheus Operator CPU request' => ['requests: {cpu: 35m, memory: 64Mi}, limits: {cpu: 200m, memory: 128Mi}', 'requests: {cpu: 36m, memory: 64Mi}, limits: {cpu: 200m, memory: 128Mi}'],
  'wrong Alertmanager CPU request' => ['requests: {cpu: 35m, memory: 64Mi}, limits: {cpu: 100m, memory: 128Mi}', 'requests: {cpu: 36m, memory: 64Mi}, limits: {cpu: 100m, memory: 128Mi}'],
  'wrong Grafana CPU request' => ['requests: {cpu: 75m, memory: 256Mi}', 'requests: {cpu: 76m, memory: 256Mi}'],
  'wrong KSM CPU request' => ['requests: {cpu: 35m, memory: 64Mi}, limits: {cpu: 200m, memory: 128Mi}', 'requests: {cpu: 36m, memory: 64Mi}, limits: {cpu: 200m, memory: 128Mi}'],
  'wrong node-exporter CPU request' => ["- name: node-exporter\n          resources: {requests: {cpu: 15m, memory: 32Mi}, limits: {cpu: 100m, memory: 64Mi}}", "- name: node-exporter\n          resources: {requests: {cpu: 16m, memory: 32Mi}, limits: {cpu: 100m, memory: 64Mi}}"],
  'wrong Loki CPU request' => ['requests: {cpu: 120m, memory: 256Mi}', 'requests: {cpu: 121m, memory: 256Mi}'],
  'wrong Alloy CPU request' => ['requests: {cpu: 60m, memory: 128Mi}', 'requests: {cpu: 61m, memory: 128Mi}'],
  'wrong Alloy reloader CPU request' => ['requests: {cpu: 5m, memory: 50Mi}', 'requests: {cpu: 6m, memory: 50Mi}'],
  ...
  'Alertmanager memory request' => ['requests: {cpu: 35m, memory: 64Mi}', 'requests: {cpu: 35m, memory: 32Mi}'],
  ...
  'missing Alloy pod label' => ['"cluster", "namespace", "app", "container", "pod", "stream"', '"cluster", "namespace", "app", "container", "stream"'],
  'missing Alloy pod relabel' => ["    discovery.relabel \"pod_logs\" {\n      rule { source_labels = [\"__meta_kubernetes_pod_name\"] action = \"replace\" target_label = \"pod\" }\n    }\n", ''],
  'unapproved Alloy label' => ['"cluster", "namespace", "app", "container", "pod", "stream"', '"cluster", "namespace", "app", "container", "pod", "stream", "job"'],
