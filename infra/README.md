...
`infra/scripts/validate.sh` là static validator và không render Helm chart. Semantic validation có hai lớp: `infra/scripts/render-observability.sh` yêu cầu Helm `v3.21.3` (cho phép build metadata như `+g1ad6e68`), render ba chart pin rồi gọi `infra/scripts/validate-rendered-observability.rb`; Ruby validator xử lý YAML đã render theo trusted pre-render input boundary và từ chối explicit Ruby object/class/module tag. Vì `kube-prometheus-stack` render thực tế không phát hành `helm.sh/chart`, chart version được chứng minh trung thực bằng exact URL, archive filename và SHA-256 trong `infra/scripts/test-real-render-observability.sh`; validator chỉ kiểm tra release và app metadata thực sự quan sát được. Driver này tải Helm và ba chart archive từ URL chính thức vào thư mục tạm, kiểm tra SHA-256 pin, render, chạy validator và mutation suite, rồi luôn dọn dữ liệu tạm. Có thể chạy wrapper với `--chart-dir` chứa đúng ba chart archive local để tránh truy cập Helm repository.

Workflow `infra-quality` chạy static validator, fixture suite, workflow execution/policy tests, rồi `infra/scripts/test-real-render-observability.sh`; vì vậy CI thực hiện real render từ ba official archive đã checksum-pin trước khi deployment. Job `deploy-observability` không dùng wrapper này; Ansible thực hiện deployment trên VPS theo chart pin.
