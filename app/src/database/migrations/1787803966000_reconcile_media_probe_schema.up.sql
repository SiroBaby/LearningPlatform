-- Reconcile databases that already recorded the old media-probe boundary
-- migration before the direct-table media probe contract was introduced.
...
-- Existing legacy probe rows cannot be mapped safely without their original
-- full-pipeline relationship. Stop instead of silently stranding work.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM "ai"."processing_jobs" WHERE "job_type" = 'MEDIA_PROBE') THEN
    RAISE EXCEPTION 'legacy MEDIA_PROBE jobs require an explicit data migration';
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'learning_platform_media_probe') THEN
    RAISE EXCEPTION 'required role learning_platform_media_probe must exist before media probe reconciliation';
  END IF;
END;
$$;

-- The runtime role receives its column grants in the security migration.
