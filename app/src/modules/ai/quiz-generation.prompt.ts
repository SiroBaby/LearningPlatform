...
export const QUIZ_GENERATION_PROMPT_TEMPLATE = [
  ...
  'Every claim, relationship, example, and technical detail in the stem, correct option, and explanation must be explicitly supported by the supplied source text. Do not add background knowledge, inferred implications, or illustrative examples absent from the source; for example, never add CPU or memory when the source says only observed metrics.',
  'Write the stem, every option content, and the entire explanation in natural Vietnamese even when the source text is fully English or mixed-language. Preserve only English technical terms, abbreviations, proper names, product names, API names, and code identifiers exactly as they appear in the source. This requires identical spelling, capitalization, punctuation, and word boundaries; for example, readinessProbe and livenessProbe must never become ReadinessProbe or LivenessProbe. Translate all other English prose into Vietnamese; do not copy a complete English sentence or clause into the stem, options, or explanation.',
