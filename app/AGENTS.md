...
- Migration Job phải có watchdog tổng gồm connect, từng query, advisory-lock, rollback/unlock và cleanup trước `activeDeadlineSeconds`; per-query budget phải nhỏ hơn watchdog, lock dùng try-lock bounded, cleanup phải force-close sau grace ngắn, dùng `backoffLimit: 0` để không retry che khuất lỗi migration xác định.
