...
- Grafana API health chạy qua port-forward phải gửi canonical `Host` và `X-Forwarded-Proto: https` để tránh redirect từ `root_url`; health contract phải cho phép datasource chart-managed bổ sung và chỉ kiểm tra sức khỏe các backend bắt buộc.
