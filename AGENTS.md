...
- Khi thay thế Kubernetes Job cùng tên, chỉ correlate Pod và terminal result với exact `metadata.uid` của Job vừa apply (controller UID); không dùng lại `job-name` vì Pod cũ có thể còn tồn tại.
