...
try: result["dashboards"]=sum(x in {x.get("title") for x in get("/api/search?type=dash-db")} for x in {"Kubernetes / Compute Resources / Cluster","Kubernetes / Compute Resources / Node (Pods)","Kubernetes / Compute Resources / Namespace (Pods)","Node Exporter / Nodes","Application / Pod"})
except Exception: pass
...
[[ "$releases" == true && "$(python3 -c 'import json,sys; print(str(json.load(sys.stdin)["valid"]).lower())' <<<"$pvc_json")" == true && "$pods" == true && "$ksm" == true && "$prometheus" == true && "$marker_latency" -le 120 && "$grafana_health" == true && "$datasources" == true && "$dashboards" == 5 ]] || mark_failed
