...
grafana_contract="$(kubectl_ro --namespace "$OBSERVABILITY_NAMESPACE" get secret grafana-admin -o json | python3 -c 'import base64,json,sys,urllib.request; result={"health":False,"datasources":False,"dashboards":0};
try:
 s=json.load(sys.stdin)["data"]; token=base64.b64encode(base64.b64decode(s["admin-user"])+b":"+base64.b64decode(s["admin-password"])).decode(); get=lambda path: json.load(urllib.request.urlopen(urllib.request.Request("http://127.0.0.1:13000"+path,headers={"Authorization":"Basic "+token}),timeout=10)); result["health"]=get("/api/health").get("database")=="ok"; ds={x.get("name"):(x.get("type"),x.get("url"),x.get("id")) for x in get("/api/datasources")}; expected={"Prometheus":("prometheus","http://prometheus-operated.observability.svc.cluster.local:9090"),"Loki":("loki","http://learning-platform-loki.observability.svc.cluster.local:3100")}; result["datasources"]=all(name in ds and ds[name][:2]==contract and get("/api/datasources/%s/health" % ds[name][2]).get("status")=="OK" for name,contract in expected.items()) and len(ds)==2; titles={x.get("title") for x in get("/api/search?type=dash-db")}; result["dashboards"]=sum(x in titles for x in {"Kubernetes / Compute Resources / Cluster","Kubernetes / Compute Resources / Node (Pods)","Kubernetes / Compute Resources / Namespace (Pods)","Node Exporter / Nodes"})
