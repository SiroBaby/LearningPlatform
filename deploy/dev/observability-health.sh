...
grafana_contract="$(kubectl_ro --namespace "$OBSERVABILITY_NAMESPACE" get secret grafana-admin -o json | python3 -c 'import base64,json,sys,urllib.request; result={"health":False,"datasources":False,"dashboards":0}; s=json.load(sys.stdin)["data"]; token=base64.b64encode(base64.b64decode(s["admin-user"])+b":"+base64.b64decode(s["admin-password"])).decode(); get=lambda path: json.load(urllib.request.urlopen(urllib.request.Request("http://127.0.0.1:13000"+path,headers={"Authorization":"Basic "+token}),timeout=10));
try: result["health"]=get("/api/health").get("database")=="ok"
except Exception: pass
try:
 ds={x.get("name"):x for x in get("/api/datasources")}; expected={"Prometheus":("prometheus","http://prometheus-operated.observability.svc.cluster.local:9090"),"Loki":("loki","http://learning-platform-loki.observability.svc.cluster.local:3100")}; result["datasources"]=len(ds)==2 and all(name in ds and (ds[name].get("type"),ds[name].get("url"))==contract and ds[name].get("uid") and get("/api/datasources/uid/%s/health" % ds[name]["uid"]).get("status")=="OK" for name,contract in expected.items())
except Exception: pass
